...
Every script below is invoked from the project root, since
`.claude/state/team-tasks.json` is resolved relative to the current working
directory (see `task-state.sh`'s own header comment).
...
`scripts/team/complete-gate.sh <id>`. Not by you editing state, not by a
builder's or verifier's claim of "done," not by `task-state.sh complete`
...
1. Run `bash scripts/team/task-state.sh status <id>`.
...
3. If it fails ("task '<id>' not found"), the task record does not exist
   ...
   c. Create the record:
      `bash scripts/team/task-state.sh create <id> "<title>" --depends <dep-ids-or-omit> --risk <low|medium|high> --budget <N>`
...
Run `bash scripts/team/task-state.sh start <id>`.
...
Run:
```
bash scripts/team/assign.sh <id> --role builder --agent-type team-builder \
  [--skills path1,path2,...]
```
...
3. `agents/team-builder.md`'s own stated workflow, made explicit again in
   ...
   (`bash scripts/team/task-state.sh check <id>`); **never call
...
`bash scripts/team/task-state.sh status <id>`
...
Run:
```
bash scripts/team/assign.sh <id> --role verifier --agent-type team-verifier \
...
1. Independently confirm evidence actually landed — do not rely on the
   ...
   `bash scripts/team/task-state.sh status <id>` and check the `evidence`
   array is non-empty and its last entry's `recorded_at` is recent (from
   this verification pass, not a stale entry from a much earlier builder
...
2. If a fresh evidence entry exists, run the gate as the real completion
   attempt:
   `bash scripts/team/complete-gate.sh <id>` (add `--allow-no-tests` only
   if this part genuinely has no test surface — do not use it to dodge a
